#!/usr/bin/env python3
"""
One-shot setup for UNSUBSCRIBE_SECRET — the HMAC secret used to sign unsubscribe tokens in
email-link generation, verified by /api/unsubscribe.
...
dedicated setup-*.py script. UNSUBSCRIBE_SECRET differs from every other secret in
...
Flow:
  ...
  3. Pipe the value to `vercel env add UNSUBSCRIBE_SECRET <env> --sensitive --yes`
...
Usage:
  python3 scripts/setup-unsubscribe-secret.py                # production only
  python3 scripts/setup-unsubscribe-secret.py --env all      # all envs + local file
  python3 scripts/setup-unsubscribe-secret.py --env preview  # preview only (rare)
"""
...
def ensure_linked() -> None:
    ...
    if project_json.exists():
        return
    print(f"[setup-unsubscribe-secret] linking worktree to {VERCEL_PROJECT}…")
...
def push_to_vercel(env_target: str, value: str) -> None:
    """
    Add UNSUBSCRIBE_SECRET to a single Vercel env (production | preview | development).
    ...
    print(f"[setup-unsubscribe-secret] pushing {KEY_NAME} to Vercel {env_target}…")
    result = subprocess.run(
        args,
        cwd=REPO_ROOT,
        input=value + "\n",
...
def write_local(value: str) -> None:
    """Persist UNSUBSCRIBE_SECRET in .env.development.local so local cron testing works."""
...
def main() -> None:
    ...
    # want.
    print()
    print("[setup-unsubscribe-secret] verifying via `vercel env ls`…")
    ls = subprocess.run(
        ["vercel", "env", "ls"],
        cwd=REPO_ROOT,
        capture_output=True,
    ...
    if not matches:
        sys.exit(
            "WARN: vercel env ls did not list UNSUBSCRIBE_SECRET — push may have "
    ...
    print()
    print("✓ Done. The unsubscribe HMAC token signer now has a stable secret.")
    print("  Existing unsubscribe links signed with the prior secret will be invalidated.")
