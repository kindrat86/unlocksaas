...
/**
 * /llms-perplexity.txt – per-model static URL variant of /llms.txt for Perplexity retrievers.
 *
 * Why this dedicated route exists
 * -------------------------------
 * /llms.txt honours `?model=perplexity`, but some retrievers and registry crawlers
 ...
 *
 * The body is identical to GET /llms.txt?model=perplexity: the curated body
 * produced by `getCachedLlmsTxtForModel("perplexity")` from the shared
...
export function GET() {
  const body = getCachedLlmsTxtForModel("perplexity");
  return new NextResponse(body, {
    ...
    headers: {
      ...
      "x-llms-model-resolved": "perplexity",
    },
  });
}
