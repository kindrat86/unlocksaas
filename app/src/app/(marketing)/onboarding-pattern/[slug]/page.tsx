...
import {
  ONBOARDING_PATTERN_SLUGS,
  ONBOARDING_PATTERN_ENTRIES,
  getOnboardingPatternBySlug,
  type OnboardingPatternEntry,
...
export function generateStaticParams() {
  return ONBOARDING_PATTERN_SLUGS.map((slug) => ({ slug }));
}
...
export async function generateMetadata(props: {
  ...
  const e = getOnboardingPatternBySlug(params.slug);
  if (!e) return {};
  const canonical = `/onboarding-pattern/${e.slug}`;
  return {
    title: e.metaTitle,
...
export default async function OnboardingPatternDetailPage(props: {
  params: Promise<RouteParams>;
}) {
  const params = await props.params;
  const e = getOnboardingPatternBySlug(params.slug);
  ...
  const canonicalUrl = `${BASE_URL}/onboarding-pattern/${e.slug}`;
  const [articleJson, faqJson, breadcrumbJson] = buildJsonLd(e, canonicalUrl);

  const related = ONBOARDING_PATTERN_ENTRIES.filter(
    ...
    <article className="min-h-screen">
      ...
      >
        <ol className="flex items-center gap-2">
          ...
          <li>
            <Link href="/onboarding-pattern" className="hover:underline">
              Onboarding patterns
            </Link>
          </li>
          <li aria-hidden="true">/</li>
      ...
      >
        <h2 id="metric" className="text-xl font-semibold mb-4 leading-tight">
          Activation metric
        </h2>
        <p className="text-base leading-relaxed">{e.activationMetric}</p>
      </section>
      ...
      >
        ...
        <ul className="space-y-3 list-disc list-inside">
          {e.variations.map((v) => (
            <li key={v} className="text-base leading-relaxed">
              {v}
            </li>
          ))}
        </ul>
        ...
        >
          ...
          <ul className="space-y-2">
            {related.map((r) => (
              <li key={r.slug}>
                <Link
                  href={`/onboarding-pattern/${r.slug}`}
        ...
        <Card className="border-primary/40 bg-primary/5">
          <CardContent className="pt-6 pb-6">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed mb-5">
              The right onboarding pattern is product-specific. Test the
              activation rate before fully committing.
            </p>
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <Button asChild variant="outline">
                <Link href="/onboarding-pattern">All patterns</Link>
              </Button>
            </div>
          </CardContent>
        </Card>
