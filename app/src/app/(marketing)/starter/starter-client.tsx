...
// Resolved searchParams shape passed from the parent server component.
// Next.js's App Router gives searchParams as `Record<string, string | string[] | undefined>`;
// we accept that exact shape so callers can drop the awaited value in unmodified.
export type StarterSearchParams = Record<string, string | string[] | undefined>;

/** Read a single string value from a Next.js searchParams record. */
function readParam(searchParams: StarterSearchParams, key: string): string | null {
...
function DiagnosticHandoffBanner({
  ...
  const bucketKey = readParam(searchParams, "bucket") ?? "";
  const labelKey = readParam(searchParams, "label") ?? "";
  const copy =
...
/**
 * Starter sales page client component.
 *
 * Takes searchParams as a server-resolved prop (NOT via useSearchParams())
 * so the parent server component can fully render the static page shell
 * during SSR. The previous implementation used `<Suspense fallback={null}>`
 * wrapping a useSearchParams() consumer, which made the SSR output an
 * empty shell for every crawler — 18 of 19 tested AI/search user-agents
 * received zero rendered content (caught by the 2026-05-22 crawler
 * citation audit; only Applebot saw real content because its responses
 * were marked `cache-control: private, no-store` and bypassed the CDN
 * cache of the empty render).
 *
 * Passing searchParams as a prop removes the suspension point entirely:
 * the server already has the URL params at request time, renders the
 * full page (including the diagnostic handoff banner if applicable),
 * and ships complete HTML to every crawler.
 */
export function StarterSalesClient({
  ...
  const attribution = useMemo(() => {
    const from = readParam(searchParams, "from");
    const label = readParam(searchParams, "label");
    const bucket = readParam(searchParams, "bucket");
    const lead = readParam(searchParams, "lead");
  ...
  return (
    <div className="min-h-screen py-12 sm:py-16 px-4 sm:px-6">
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <DiagnosticHandoffBanner searchParams={searchParams} />
