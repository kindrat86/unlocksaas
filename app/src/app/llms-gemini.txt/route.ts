...
/**
 * /llms-gemini.txt – per-model static URL variant of /llms.txt for Gemini retrievers.
 *
 * Why this dedicated route exists
 * -------------------------------
 * /llms.txt honours `?model=gemini`, but some retrievers and registry crawlers
 ...
 *
 * The body is identical to GET /llms.txt?model=gemini: the curated body
 * produced by `getCachedLlmsTxtForModel("gemini")` from the shared
...
export function GET() {
  const body = getCachedLlmsTxtForModel("gemini");
