 ...
 * × GLOSSARY_SLUGS. The canonical slug list is the source of truth — a
 * locale page exists for every approved/pending-review (locale, slug)
 * pair, with text overlaid from glossary.es.ts / glossary.pt-br.ts.
 *
 * JSON-LD posture matches canonical:
 *   - DefinedTerm with @id at `{canonical}/{slug}#term` and
 *     `inDefinedTermSet` pointing at the en-US canonical set.
 *   - Article + FAQPage + BreadcrumbList, all with inLanguage matching
 *     the locale.
 */
...
export default async function LocalizedGlossaryDetail({
  ...
  // Related terms come from the canonical row (we didn't carry them in the
  // overlay because slug refs must stay aligned with the en-US source).
  // resolveRelated reads from canonical GLOSSARY.
