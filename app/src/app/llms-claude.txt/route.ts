...
/**
 * /llms-claude.txt – per-model static URL variant of /llms.txt for Claude retrievers.
 *
 * Why this dedicated route exists
 * -------------------------------
 * /llms.txt honours `?model=claude`, but some retrievers and registry crawlers
 ...
 *
 * The body is identical to GET /llms.txt?model=claude: the curated body
 * produced by `getCachedLlmsTxtForModel("claude")` from the shared
...
export function GET() {
  const body = getCachedLlmsTxtForModel("claude");
