...
1. **Execute Week 1 of `strategy/dream-100-outreach.md` §1 cadence.** The deployable kit is shipped: 20 per-influencer dossiers (handle + recent canon + warm-up reps + ready-to-send DM template), 5 Tier-1 podcast pitch templates (gated on first verified customer), 5 integration partner pitches (gated on 3+ cycles), affiliate one-pager (gated on 50+ customers), tracking table schema, 4-week cadence. **Week 1 targets** (Tier A — highest ICP overlap): Anthony Castrio, Marc Lou, Damon Chen, Arvid Kahl, Mubashar Iqbal. **Mon-Fri shape:** Mon read their last 30 days; Tue public reply on each newest post; Wed public reply on each 30d-old post; Thu DM using the template from each §2 dossier (fill one blank — the specific recent post you reference); Fri Indie Hackers long-form (parable-led from workbook 01 §6 Beat 3, no link in body). Log every touch in the §5 table. Per-message Maryan confirmation enforced — kit is "ready to send," not "sent."
